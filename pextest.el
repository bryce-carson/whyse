...
@nl
@text This section covers the actual parsing of the tool syntax. Production of
@nl
@text the tool syntax from a Noweb project is covered in §\ref{Projects}. The
@nl
@text following LISP code uses the \textsc{peg} Emacs Lisp package to provide
@nl
@text a parser that matches the lexed tokens and executes actions.
@nl
@text 
@nl
@text \begin{center}
@nl
@text   Learning the PEG library was somewhat of a challenge, and deriving a
@nl
@text   working set of PEXes, rules, for parsing the Noweb tool syntax
@nl
@text   produced by \texttt{markup} was difficult. For this reason, the first
@nl
@text   working version of a grammar which parses a subset of the tool syntax
@nl
@text   is memorialized both in the git repository and in the 
@quote
@xref ref NW4UAmWz-4VHk5F-1
@use test-pex
@endquote
@text 
@nl
@text   chunk in the appendices of this document.
@nl
@text \end{center}
@nl
@text 
...
@text            (lambda (lst)
@nl
@text              (message "Parsing failed in buffer:=%S.\nPEG expressions which failed:=%S" buffer lst))))
@nl
@end code 28
@begin docs 29
@text 
...
@text noweb
@endquote
@text  rule defines the root expression, or starting expression,
@nl
@text for the grammar. The tool syntax of Noweb is simply a list of one or
@nl
@text more files, which are each composed of at least one chunk.
@nl
@text 
@nl
@end docs 29
...
@xref ref NW4UAmWz-CjdUq-1
@defn PEG rules
@xref nextdef NW4UAmWz-CjdUq-2
@xref begindefs
@xref defitem NW4UAmWz-CjdUq-2
@xref defitem NW4UAmWz-CjdUq-3
@xref defitem NW4UAmWz-CjdUq-4
@xref defitem NW4UAmWz-CjdUq-5
@xref defitem NW4UAmWz-CjdUq-6
@xref defitem NW4UAmWz-CjdUq-7
@xref defitem NW4UAmWz-CjdUq-8
@xref enddefs
@xref beginuses
...
@use test-pex
@endquote
@text  works, the main PEG defined in the Noweb
@nl
@text file does not work. It fails very early.
@nl
@text 
@nl
@end docs 31
@begin code 32
@xref label NW4UAmWz-CjdUq-2
@xref ref NW4UAmWz-CjdUq-1
@defn PEG rules
@xref prevdef NW4UAmWz-CjdUq-1
@xref nextdef NW4UAmWz-CjdUq-3
@nl
@text ;; Helpers
@nl
...
@end docs 33
@begin code 34
@xref label NW4UAmWz-CjdUq-3
@xref ref NW4UAmWz-CjdUq-1
@defn PEG rules
@xref prevdef NW4UAmWz-CjdUq-2
@xref nextdef NW4UAmWz-CjdUq-4
@nl
@text (file (bol) "@file" [space] path (eol) "\n"
@nl
@text       (list (+ chunk)))
@nl
@end code 34
...
@end docs 35
@begin code 36
@xref label NW4UAmWz-CjdUq-4
@xref ref NW4UAmWz-CjdUq-1
@defn PEG rules
@xref prevdef NW4UAmWz-CjdUq-3
@xref nextdef NW4UAmWz-CjdUq-5
@nl
@text ;;; Valid filenames the PEX was tested against.
@nl
@text ;; /home/bryce/src/
@xref ref NW4UAmWz-4YCtgL-1
@index use whs
...
@text .nw
@nl
@text (path (substring (opt (or ".." ".")) (* path-component) file-name))
@nl
...
@end docs 37
@begin code 38
@xref label NW4UAmWz-CjdUq-5
@xref ref NW4UAmWz-CjdUq-1
@defn PEG rules
@xref prevdef NW4UAmWz-CjdUq-4
@xref nextdef NW4UAmWz-CjdUq-6
@nl
...
@nl
@text      `(k1 z1 keywords k2 z2 --
@nl
@text           (if (and (= z1 z2) (string= k1 k2))
@nl
...
@text 
@nl
@text The result of parsing the beginning and end of a chunk are pushed onto
@nl
@text the stack as a list.
@nl
@text 
@nl
...
@nl
@text be popped off of it. The push operations in 
@quote
@text kind
@endquote
@text  and 
...
@text 
@nl
@text implement the beginning of a chunk, and the stack actions in the 
@quote
@text end
@endquote
@text 
@nl
@text rule check that the chunk elements on the stack are balanced.
@nl
@text 
@nl
@text The PEG library's stack operations may be a little confusing, so they
@nl
@text will be discussed now.
@nl
@text 
@nl
@text %% FIXME: verify this behaviour by examining the code of the PEG library.
@nl
@text Stack manipulating operations, such as 
@quote
@text substring
@endquote
@text , push their
@nl
@text matching PEX input text to the stack. Labels in the left half of the
@nl
@text stack actions pop elements off of the stack and let-bind them, making
@nl
@text them easier to use in the right hand side. The right hand side uses
@nl
@text let-bound variables and pushes its results back to the stack. Further,
@nl
@text it is valid to pop elements of the stack without pushing them back
@nl
@text (discarding them); it is also valid to push elements to the stack
@nl
@text without popping anything off the stack.
@nl
@text 
@nl
@text Further rules needed to parse a chunk are an ordinal and the kind of
@nl
@text chunk: code or documentation. An ordinal, in this sense, is any positive
@nl
@text integer; it is a counting number. The kinds of chunks are represented by
@nl
@text the strings ``code'' and ``docs''.
@nl
@text 
@nl
@text %% TODO: introduce the structural, tagging, and error keywords with a small amount of prose.
@nl
@end docs 39
@begin code 40
@xref label NW4UAmWz-CjdUq-6
@xref ref NW4UAmWz-CjdUq-1
@defn PEG rules
@xref prevdef NW4UAmWz-CjdUq-5
@xref nextdef NW4UAmWz-CjdUq-7
@nl
@text (keyword
@nl
@text  (substring (or
...
@end docs 41
@begin code 42
@xref label NW4UAmWz-CjdUq-7
@xref ref NW4UAmWz-CjdUq-1
@defn PEG rules
@xref prevdef NW4UAmWz-CjdUq-6
@xref nextdef NW4UAmWz-CjdUq-8
@nl
@text ;; strings containing spaces must match: "filename containing spaces.txt"
@nl
...
@nl
@nl
@text (defn "@defn" [space] (substring many-before-eol)
@nl
@text   `(cdefn -- (cons "Chunk definition" cdefn)))
@nl
@nl
@text (use (bol) "@use" [space] (substring many-before-eol))
@nl
@text (beginquote (bol) "@quote" (substring many-before-eol))
@nl
@text (endquote (bol) "@endquote" (substring many-before-eol))
@nl
@nl
...
@nl
@text (index (bol) "@index" (opt (and [space] index-keyword)) new-line)
@nl
@text (xref (bol) "@xref" (opt (and [space] xref-keyword)) new-line)
@nl
@nl
...
@end docs 43
@begin code 44
@xref label NW4UAmWz-CjdUq-8
@xref ref NW4UAmWz-CjdUq-1
@defn PEG rules
@xref prevdef NW4UAmWz-CjdUq-7
@nl
@text (i-defn "defn" [space] many-before-eol)
@nl
@text (i-localdefn "localdefn" [space] many-before-eol)
@nl
@text (i-use "use" [space] many-before-eol)
@nl
@text (i-nl "nl" [space] many-before-eol)
@nl
@nl
@text (i-begindefs "begindefs")
@nl
@text (i-isused "isused" [space] many-before-eol)
@nl
@text (i-defitem "defitem" [space] many-before-eol)
@nl
@text (i-enddefs "enddefs")
@nl
@nl
@text (i-beginuses "beginuses")
@nl
@text (i-isdefined "isdefined" [space] (+ [word]))
@nl
@text (i-useitem "useitem" [space] many-before-eol)
@nl
...
@nl
@text (i-entrybegin "entrybegin" [space] (+ [word]) [space] many-before-eol)
@nl
@text (i-entryuse "entryuse" [space] (+ [word]))
@nl
@text (i-entrydefn "entrydefn" [space] (+ [word]))
@nl
...
@text (x-prevdef
@nl
@text   "prevdef" [space] (+ [word]))
@nl
@text (x-nextdef
@nl
@text   "nextdef" [space] (+ [word]))
@nl
@nl
@text (x-beginuses
@nl
...
@text (x-notused
@nl
@text   "notused" [space] many-before-eol)
@nl
@nl
@text (x-beginchunks
@nl
...
@text (x-chunkbegin
@nl
@text   "chunkbegin" [space] (+ [word]) [space] many-before-eol)
@nl
@text (x-chunkuse
@nl
@text   "chunkuse" [space] (+ [word]))
@nl
@text (x-chunkdefn
@nl
@text   "chunkdefn" [space] (+ [word]))
@nl
@text (x-chunkend
@nl
@text   "chunkend")
...
@nl
@nl
@text ;; Associates label with tag (word with many-before-eol)
@nl
@text (x-tag
@nl
@text   "tag" [space] (+ [word]) [space] many-before-eol)
@nl
@nl
@text ;; User-errors (header and trailer) and tool-error (fatal)
@nl
@text (header (bol) "@header" [space] (+ (not (null))) new-line
@nl
@text         (action (error 
@xref label NW4UAmWz-CjdUq-8-u1
@xref ref NW4UAmWz-2AOWdv-1
@use user error text
@text )))
@nl
@text (trailer (bol) "@trailer" [space] (+ (not (null))) new-line
@nl
@text          (action (error 
@xref label NW4UAmWz-CjdUq-8-u2
@xref ref NW4UAmWz-2AOWdv-1
@use user error text
@text )))
@nl
@text (fatal (bol) "@fatal" (* (any))
@nl
@text        (action (error 
@xref label NW4UAmWz-CjdUq-8-u3
@xref ref NW4UAmWz-4AiICF-1
@use tool error text
@text )))
@nl
@end code 44
...
@end docs 45
@begin code 46
@xref label NW4UAmWz-2AOWdv-1
@xref ref NW4UAmWz-2AOWdv-1
@defn user error text
@xref beginuses
@xref useitem NW4UAmWz-CjdUq-8
@xref enduses
@nl
@text "User error. Do not use totex or tohtml in your noweave pipeline."
@nl
@end code 46
@begin docs 47
@text 
...
@xref enduses
@nl
@text "There was a fatal error in the pipeline. Debug the tools."
@nl
@end code 48
@begin docs 49
@text 
...
@text 
@nl
@text \subsection{Testing the PEX grammar for Noweb's tool syntax}
@nl
@end docs 65
@begin code 66
@xref label NW4UAmWz-4VHk5F-1
@xref ref NW4UAmWz-4VHk5F-1
@defn test-pex
@xref notused test-pex
@nl
@text @file /home/bryce/src/
@xref ref NW4UAmWz-4YCtgL-1
@index use whs
@text whs
@text /
...
@text @begin docs 0
@nl
@text @text The Bluetooth Device has been connected successfully.
@nl
@text @nl
@nl
@text @text The Bluetooth Device has been connected successfully.
...
@text @begin docs 0
@nl
@text @text Hello, world of literate programming.
@nl
@text @nl
@nl
@text @end docs 0
...
@text              (list (+ chunk))
@nl
@text              `(filename chunks -- (cons filename chunks)))
@nl
@nl
@text        ;;; Valid filenames the PEX was tested against.
@nl
...
@nl
@nl
@text        ;; Chunk keyword definitions for what is in pextest.el (this file).
@nl
@text        (chunk begin
@nl
@text               ;; keywords
@nl
@text               (list (* (or text nl)))
@nl
@text               end)
@nl
@text        (text (bol) "@text" [space] (substring (* (and (not "\n") (any)))) (eol) "\n")
...
@text ;; Local Variables:
@nl
@text ;; major-mode: lisp-interaction
@nl
@text ;; End:
@nl
@index begindefs
@index isused NW4UAmWz-4VHk5F-1
@index defitem test-pex
@index enddefs
@index beginuses
@index isdefined NW4UAmWz-4YCtgL-1
@index isdefined NW4UAmWz-2Oqtqm-1
...
@xref chunkdefn NW4UAmWz-CjdUq-2
@xref chunkdefn NW4UAmWz-CjdUq-3
@xref chunkdefn NW4UAmWz-CjdUq-4
@xref chunkdefn NW4UAmWz-CjdUq-5
@xref chunkdefn NW4UAmWz-CjdUq-6
@xref chunkdefn NW4UAmWz-CjdUq-7
@xref chunkdefn NW4UAmWz-CjdUq-8
@xref chunkend
...
@xref chunkend
@xref chunkbegin NW4UAmWz-4VHk5F-1 test-pex
@xref chunkdefn NW4UAmWz-4VHk5F-1
@xref chunkend
@xref chunkbegin NW4UAmWz-4AiICF-1 tool error text
@xref chunkuse NW4UAmWz-CjdUq-8
@xref chunkdefn NW4UAmWz-4AiICF-1
@xref chunkend
@xref chunkbegin NW4UAmWz-2AOWdv-1 user error text
@xref chunkuse NW4UAmWz-CjdUq-8
@xref chunkdefn NW4UAmWz-2AOWdv-1
@xref chunkend
...
@xref endchunks
@index beginindex
@index entrybegin NW4UAmWz-4VHk5F-1 test-pex
@index entrydefn NW4UAmWz-4VHk5F-1
@index entryend
...
(defun test-pex ()
  (with-peg-rules
      ...
      ((noweb (bob) (+ file))
       ...
       (defn "@defn" [space] (substring many-before-eol)
         `(cdefn -- (cons "Chunk definition" cdefn)))

       ;; FIXME: current pain point.
       (use (bol) "@use" [space] (action (message "\"@use \" matched"))
            (substring many-before-eol)
            `(chunk-name -- (cons "Chunk usage (child)" chunk-name)))

       ;; FIXME: quotations need to be grouped up.
